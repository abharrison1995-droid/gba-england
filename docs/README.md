...
| [plans/ROYAL_FIGHT_ARENA_PLAN.md](plans/ROYAL_FIGHT_ARENA_PLAN.md) | The Royal Fight Arena under the castle, hosted by Prince Mandrew: an arena chunk, ranked matches with escalating opponents, titles, and a `.arena` ladder pipeline. **Plan only — nothing implemented.** Phase 0 is a hard gate on whether NavMesh works inside an interior chunk. |

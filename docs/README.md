...
| [plans/PLAYER_SPECIAL_ATTACKS_PLAN.md](plans/PLAYER_SPECIAL_ATTACKS_PLAN.md) | Spin and dash special attacks on the right HUD row: authored as `AbilityData` but executed as melee, never through `SpellRuntime`, and deliberately kept out of `Resources/` so they never become save keys. Adds a shared melee sweep helper. **Implemented and merged to `main`** (12 commits, `649f28c`..`b188090`); unseen by a compiler or the editor, and the two ability assets do not exist until the owner runs the tool. Its §11 is the owner's check list. |

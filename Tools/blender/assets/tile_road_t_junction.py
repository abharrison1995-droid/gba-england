"""T-Junction Road Tile (10.0m x 10.0m Modular 3-Way Intersection).

Specs:
- 10.0m x 10.0m flat T-junction tile: Continuous North-South through-road (Y: -5 to +5) met by an Eastward side road (X = +3 to +5).
- Left pavement continuous (X: -5 to -3), Top-Right and Bottom-Right corner pavements with tactile pads and give-way dashed line.
- Outputs to Tools/blender/out/Background Assets/ and Tools/out/Background Assets/.

Run: python Tools/blender/bpy_runner.py Tools/blender/assets/tile_road_t_junction.py
"""
...
R_JUNCTION_ASPHALT = (0,   256, 256, 256)   # Asphalt with T-junction give way line
...
def paint_t_junction_atlas():
    a = Atlas(S, seed=701)

    # 1. T-Junction Asphalt (R_JUNCTION_ASPHALT)
    x, y, w, h = R_JUNCTION_ASPHALT
    a.rect(x, y, w, h, ASPHALT_BASE)
    # Continuous Center Broken Dashes along North-South (through road)
    dash_w = 8
    mid_x = x + (w - dash_w) // 2
    for dy in range(y, y + h, 64):
        a.rect(mid_x, dy + 8, dash_w, 48, ROAD_WHITE)
    # Give-way double lines on East side branch
    a.rect(x + w - 32, y + (h - 64) // 2, 4, 64, ROAD_WHITE)
    a.rect(x + w - 42, y + (h - 64) // 2, 4, 64, ROAD_WHITE)
    # Double yellow line on continuous West pavement gutter
    a.rect(x + 12, y, 5, h, DOUBLE_YELLOW)
    a.rect(x + 21, y, 5, h, DOUBLE_YELLOW)
    ...
    return a.to_image("tile_road_t_junction_atlas", OUT_DIR)
...
def main():
    kit.reset_scene()
    img = paint_t_junction_atlas()
    mat = material_for(img, "mat_tile_road_t_junction")
    ...
    # =========================================================================
    # Modular 10.0m x 10.0m T-Junction Tile
    # Base road: 10.0m x 10.0m
    # Left continuous pavement: 2.0m x 10.0m (X: -5.0 to -3.0)
    # Right top corner pavement: 2.0m x 2.0m (X: +3.0 to +5.0, Y: +3.0 to +5.0)
    # Right bottom corner pavement: 2.0m x 2.0m (X: +3.0 to +5.0, Y: -5.0 to -3.0)
    # =========================================================================

    # 1. Base Road Surface
    register_box("RoadTBase", 10.0, 10.0, 0.04, (0.0, 0.0, 0.0),
                 front=R_ROAD_PLAIN, sides=R_ROAD_PLAIN, top=R_JUNCTION_ASPHALT)

    # 2. Continuous Left Pavement (X = -4.0m, width 2.0m x 10.0m)
    register_box("PavementLeft", 2.0, 10.0, 0.14, (-4.0, 0.0, 0.0),
                 front=R_STONE_TRIM, sides=R_KERB_STONE, top=R_PAVEMENT_FLAG)

    # 3. Right Top Corner Pavement (X = +4.0m, Y = +4.0m, 2.0m x 2.0m)
    ...
    # 4. Right Bottom Corner Pavement (X = +4.0m, Y = -4.0m, 2.0m x 2.0m)
    ...
    # 5. Junction Manhole
    register_box("Manhole", 0.80, 0.80, 0.045, (0.0, 0.0, 0.0),
                 front=R_MANHOLE_COVER, sides=R_ROAD_PLAIN, top=R_MANHOLE_COVER)

    # =========================================================================
    # Finalize, Preview & Export
    # =========================================================================
    shell = kit.join(parts, "Tile_Road_T_Junction")
    kit.finalize(shell)
    kit.report_stats(shell)

    preview_path = OUT_DIR / "tile_road_t_junction_preview.png"
    kit.iso_preview(preview_path, [shell], resolution=1024)

    glb_path = OUT_DIR / "tile_road_t_junction.glb"
    kit.export_glb(glb_path, [shell])

    try:
        TOOLS_OUT_DIR.mkdir(parents=True, exist_ok=True)
        shutil.copy2(glb_path, TOOLS_OUT_DIR / "tile_road_t_junction.glb")
        shutil.copy2(preview_path, TOOLS_OUT_DIR / "tile_road_t_junction_preview.png")
        shutil.copy2(OUT_DIR / "tile_road_t_junction_atlas.png", TOOLS_OUT_DIR / "tile_road_t_junction_atlas.png")
    except Exception as e:
        print(f"[tile_road_t_junction] note: {e}")

    print("[tile_road_t_junction] generation complete.")

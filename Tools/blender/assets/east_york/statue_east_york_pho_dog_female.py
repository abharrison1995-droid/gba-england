"""East York Chinese Guardian Lioness — Female with Cub (Foo Dog / 雌石獅 — High-Poly ~1000 Tris).
...
- Lioness Anatomy & Geometry (~1,000 Triangles):
  - Sitting on muscular hindquarters with arched back and curved tufted tail
  - Powerful chest adorned with vermilion ceremonial ribbon collar and sculpted golden bell
  - Fierce protective roaring head with open jaws, fangs, tongue, and bulging eyes
  - Cascading 3D spiral mane curl clusters covering crown, nape, and shoulders
  - Left front paw gently restraining a playful 3D baby lion cub lying on its back looking up at mother
  - Right front paw rooted firmly to the pedestal with sculpted claws
...
Run: python Tools/blender/bpy_runner.py Tools/blender/assets/east_york/statue_east_york_pho_dog_female.py
"""
...
R_CUB_STONE         = (384, 256, 128, 128)   # Playful baby lion cub texture & eyes
...
def paint_pho_dog_female_atlas():
    a = Atlas(S, seed=1012)
    ...
    a.rect(x + 28, y + 50, 14, 12, STONE_CREAM)
    a.rect(x + w - 42, y + 50, 14, 12, STONE_CREAM)
    a.rect(x + 36, y + 22, 12, 10, STONE_CREAM)
    a.rect(x + w - 48, y + 22, 12, 10, STONE_CREAM)
    a.disc(x + w // 2, y + 32, 14, (0.75, 0.15, 0.12))
    ...
    # 4. Playful Baby Lion Cub (R_CUB_STONE)
    x, y, w, h = R_CUB_STONE
    a.rect(x, y, w, h, STONE_BASE)
    cx, cy = x + w // 2, y + h // 2
    a.disc(cx, cy, 28, STONE_CREAM)
    a.disc(cx - 10, cy + 6, 6, (0.1, 0.1, 0.1))
    a.disc(cx + 10, cy + 6, 6, (0.1, 0.1, 0.1))
    a.rect(cx - 8, cy - 12, 16, 8, MOUTH_DARK)
    a.noise(x, y, w, h, 0.02)
    ...
    return a.to_image("statue_east_york_pho_dog_female_atlas", OUT_DIR)
...
def main():
    kit.reset_scene()
    img = paint_pho_dog_female_atlas()
    mat = material_for(img, "mat_pho_dog_female")
    ...
    # =========================================================================
    # High-Poly Female Foo Dog with Cub (~1,000 Triangles)
    # =========================================================================
    ...
    # 2. Lioness Muscular Haunches & Sitting Hindquarters (Z = 0.85m to 1.55m)
    ...
    # 4. Front Legs & Paws
    # Right Foreleg (Rooted firmly on stone pedestal)
    leg_r = make_cylinder("ForelegR", 0.14, 0.85, segs=12, at=(0.35, -0.40, 0.85))
    leg_r.data.materials.append(mat)
    kit.map_faces_to_region(leg_r, R_LION_STONE, S)
    parts.append(leg_r)

    register_box("PawR", 0.32, 0.38, 0.18, (0.35, -0.48, 0.85), region=R_LION_STONE)

    # Left Foreleg (Resting gently above playful lion cub)
    leg_l = make_cylinder("ForelegL", 0.14, 0.65, segs=12, at=(-0.35, -0.30, 1.35))
    leg_l.data.materials.append(mat)
    kit.map_faces_to_region(leg_l, R_LION_STONE, S)
    parts.append(leg_l)

    register_box("PawL", 0.30, 0.34, 0.18, (-0.35, -0.40, 1.38), region=R_LION_STONE)

    # 5. Playful Baby Lion Cub lying on its back (Z = 0.85m to 1.35m)
    # Cub Torso
    cub_body = make_sphere("CubBody", 0.18, segs=12, rings=8, at=(-0.35, -0.40, 1.00))
    cub_body.data.materials.append(mat)
    kit.map_faces_to_region(cub_body, R_CUB_STONE, S)
    parts.append(cub_body)

    # Cub Head looking up
    cub_head = make_sphere("CubHead", 0.12, segs=10, rings=8, at=(-0.35, -0.52, 1.15))
    cub_head.data.materials.append(mat)
    kit.map_faces_to_region(cub_head, R_CUB_STONE, S)
    parts.append(cub_head)

    # Cub Paws reaching up
    for i, (cx, cy) in enumerate([(-0.45, -0.42), (-0.25, -0.42)]):
        cpaw = make_sphere(f"CubPaw_{i}", 0.06, segs=8, rings=6, at=(cx, cy, 1.22))
        cpaw.data.materials.append(mat)
        kit.map_faces_to_region(cpaw, R_LION_STONE, S)
        parts.append(cpaw)
    ...
    # 7. Lioness Sculpted Head & Protective Expression (Z = 2.05m to 2.70m)
    ...
    # 8. Sculpted Tail Resting over Back
    ...
    # =========================================================================
    shell = kit.join(parts, "Statue_East_York_Pho_Dog_Female")
    kit.finalize(shell)
    kit.report_stats(shell)

    preview_path = OUT_DIR / "statue_east_york_pho_dog_female_preview.png"
    kit.iso_preview(preview_path, [shell], resolution=1024)

    glb_path = OUT_DIR / "statue_east_york_pho_dog_female.glb"
    kit.export_glb(glb_path, [shell])

    try:
        TOOLS_OUT_DIR.mkdir(parents=True, exist_ok=True)
        shutil.copy2(glb_path, TOOLS_OUT_DIR / "statue_east_york_pho_dog_female.glb")
        shutil.copy2(preview_path, TOOLS_OUT_DIR / "statue_east_york_pho_dog_female_preview.png")
        shutil.copy2(OUT_DIR / "statue_east_york_pho_dog_female_atlas.png", TOOLS_OUT_DIR / "statue_east_york_pho_dog_female_atlas.png")
    except Exception as e:
        print(f"[statue_east_york_pho_dog_female] note: {e}")

    print("[statue_east_york_pho_dog_female] generation complete.")
